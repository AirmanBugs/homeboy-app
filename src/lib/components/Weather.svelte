<script lang="ts">
  ...
  // Get Beaufort scale number based on wind speed (m/s)
  const getBeaufortNumber = (windSpeed: number): number => {
  ...
  const getBeaufortIcon = (windSpeed: number): string => {
    return `wind-beaufort-${getBeaufortNumber(windSpeed)}`;
  };
...
<div class="h-full flex flex-col gap-2">
  ...
  {:else if weatherData}
    ...
    <div class="flex items-center justify-between">
      ...
      >
        {#each weatherDetails as detail, i}
          <div
            class="flex items-center justify-between cursor-help"
            title={detail.label}
          >
            {#if i === 0}
              <!-- Wind icon with rotation and Beaufort number overlay -->
              <span class="flex items-center justify-center flex-shrink-0 relative">
                <div
                  style="transform: rotate({weatherData.current.windDirection + 90}deg)"
                >
                  <WeatherIcon icon={detail.icon} size={50} />
                </div>
                <span
                  class="absolute bottom-0 right-0 text-xs font-bold text-white bg-slate-800/80 rounded-full px-1"
                >
                  {getBeaufortNumber(weatherData.current.windSpeed)}
                </span>
              </span>
            {:else}
        ...
        >
          ...
          <div class="flex items-center gap-0.5 text-xs text-slate-400">
            <div class="relative">
              ...
              >
                {getBeaufortNumber(hour.windSpeed)}
              </span>
            </div>
            <span class="ml-0.5">{formatNumber(hour.windSpeed, 1)}</span>
          </div>
