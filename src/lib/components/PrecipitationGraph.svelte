<script lang="ts">
  ...
  const maxPrecipitation = $derived(
    filteredData.length > 0
      ? Math.max(...filteredData.map((d) => d.precipitation), 0.5)
      : 1
  );
  ...
  const fetchNowcast = async () => {
    try {
      ...
      error =
        currentLang === "en"
          ? "Failed to load precipitation forecast"
          : "Kunne ikke laste nedbørsvarsel";
  ...
  const hasPrecipitation = $derived(
    filteredData.some((d) => d.precipitation > 0)
  );
</script>
  ...
  {:else if !hasPrecipitation}
    ...
    >
      ...
      <span
        >{currentLang === "en"
          ? "No rain expected"
          : `Det blir opphold neste ${nowcastMinutes} minutter`}</span
      >
    </div>
  {:else}
    <div class="relative h-32">
      <!-- Y-axis labels -->
      <div
        class="absolute left-0 top-0 bottom-6 flex flex-col justify-between text-xs text-slate-400"
      >
      ...
      <!-- Graph area -->
      <div
        class="absolute left-12 right-0 top-0 bottom-6 flex items-end gap-[2px]"
      >
        {#each filteredData as entry}
          ...
          <div class="flex-1 flex flex-col items-center">
            <div
              ...
              style="height: {height}%"
              title="{new Date(entry.time).toLocaleTimeString(
                currentLang === 'en' ? 'en-US' : 'nb-NO',
                {
                  hour: '2-digit',
                  minute: '2-digit',
                }
